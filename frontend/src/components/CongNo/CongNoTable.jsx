    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-5 rounded-xl border-l-8 border-emerald-500 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-gray-500 font-bold text-sm mb-1">TỔNG KHÁCH ĐANG NỢ (Phải Thu)</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100 text-gray-700">
              {list.length === 0 ? (
                <tr>
                  <td colSpan="6" className="p-8 text-center text-gray-400 font-medium">Chưa có dữ liệu công nợ nào.</td>
                </tr>
              ) : (
                list.map((item) => (
                  <tr key={item.id} className="hover:bg-orange-50/50 transition">
                    <td className="p-4">
                      ...
                      <div className="mt-1">
                        {item.loaiCongNo === 'khach_no' ? (
                          <span className="bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded text-xs font-bold border border-emerald-200">Khách Nợ Mình</span>
                        ) : (
                    ...
                    {/* Tính tiền còn lại trực tiếp luôn, không xài biến nữa cho đỡ rườm rà */}
                    <td className="p-4 text-right font-black text-red-600 text-lg">{formatMoney(item.soTienNo - item.soTienDaTra)}</td>
                    
                    <td className="p-4 text-center">
                      <span className={`px-3 py-1.5 rounded-full text-xs font-bold ${getStatusStyle(item.trangThai)}`}>
                        {item.trangThai}
                      </span>
                    </td>
                    <td className="p-4 text-center">
